...
// Estructura enriquecida para exportación a GENESIS
...
export const ExportService = {
  ...
  exportReactComponent: (payload: WidgetPayload): string => {
    ...
    // Para layouts, usar el objeto completo; para widgets simples, solo props
    const isLayout = payload.type === 'stack' || payload.type === 'grid';
    const propsData = isLayout ? payload : payload.props;
...
interface ${componentName}Props {
  data: ${JSON.stringify(propsData, null, 2).replace(/"/g, '')};
...
export const defaultProps = ${JSON.stringify(propsData, null, 2)};
`;
  },
  ...
  exportHTML: (payload: WidgetPayload): string => {
    // Para layouts, mostrar el objeto completo; para widgets simples, solo props
    const isLayout = payload.type === 'stack' || payload.type === 'grid';
    const displayData = isLayout ? payload : payload.props;
...
<body>
  <div id="widget-container" class="glass-card">
    ...
    <pre class="text-white text-xs">${JSON.stringify(displayData, null, 2)}</pre>
  </div>
